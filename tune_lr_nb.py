...
class TraditionalMLTuner:
    ...
    def _get_prepared_dataset(self, dataset_name: str, preprocessing_name: str) -> PreparedDataset:
        ...
        if cache_key not in self.preprocessed_cache:
            ...
            self.preprocessed_cache[cache_key] = PreparedDataset(dataset=dataset, processed_texts=processed_texts)
            self.logger.info(
    ...
    def _build_dataset_view(self, dataset_name: str, preprocessing_name: str) -> DatasetView:
        ...
        return DatasetView(
            ...
            train_texts=_slice_list(prepared.processed_texts, split_indices.train_indices),
            val_texts=_slice_list(prepared.processed_texts, split_indices.val_indices),
            test_texts=_slice_list(prepared.processed_texts, split_indices.test_indices),
            train_labels=np.asarray(_slice_list(dataset.labels, split_indices.train_indices), dtype=np.int64),
            val_labels=np.asarray(_slice_list(dataset.labels, split_indices.val_indices), dtype=np.int64),
            test_labels=np.asarray(_slice_list(dataset.labels, split_indices.test_indices), dtype=np.int64),
        )
    ...
    def _lr_baseline_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "penalty": str(baseline_cfg.get("penalty", "l2")),
            "solver": str(baseline_cfg.get("solver", "liblinear")),
        ...
        for c_value in c_values:
            for class_weight in class_weights:
                for solver, penalty in solver_penalty_pairs:
                    ...
                    signature = json.dumps(_normalize_for_serialization(config), sort_keys=True)
                    if signature not in seen:
    ...
    def _build_feature_set(self, view: DatasetView, vectorizer_config: dict[str, Any]) -> FeatureSet:
        ...
        if vectorizer_config["type"] == "count":
            return build_count_features(
                feature_name=feature_name,
                config=vectorizer_config,
                train_texts=view.train_texts,
                val_texts=view.val_texts,
                test_texts=view.test_texts,
            )
    ...
    def _build_estimator(self, model_name: str, model_config: dict[str, Any]) -> Any:
        if model_name == "logistic_regression":
            ...
            if penalty not in {"l1", "l2"}:
                raise ValueError(f"Unsupported Logistic Regression penalty: {penalty}")
    ...
    def _next_experiment_name(self, dataset_name: str, preprocessing_name: str, model_name: str) -> str:
        self.trial_counter += 1
        return f"{slugify(dataset_name)}_{slugify(preprocessing_name)}_{slugify(model_name)}_{self.trial_counter:05d}"
    ...
    def _evaluate_trial(
        ...
        estimator = self._build_estimator(model_name=model_name, model_config=model_config)
        ...
        return TrialResult(
            experiment_name=self._next_experiment_name(view.dataset_name, view.preprocessing_name, model_name),
    ...
    def _screen_vectorizers(self, view: DatasetView) -> list[TrialResult]:
        ...
        for index, vectorizer_config in enumerate(screen_configs, start=1):
            ...
            try:
                feature_set = self._build_feature_set(view=view, vectorizer_config=vectorizer_config)
    ...
    def _tune_model_family(
        ...
        for vectorizer_config in vectorizer_configs:
            ...
            if signature not in feature_cache:
                try:
                    feature_cache[signature] = self._build_feature_set(view=view, vectorizer_config=vectorizer_config)
                except Exception as exc:
                    self.logger.warning(
            ...
            feature_set = feature_cache[signature]
            for model_config in model_grid:
                completed += 1
                if completed == 1 or completed % 50 == 0 or completed == total:
                    self.logger.info(
                        "Running %s trial %d/%d for dataset='%s' preprocessing='%s'",
                        model_name,
                        completed,
                        total,
                        view.dataset_name,
                ...
                try:
                    ...
                    self.logger.warning(
                        ...
                        exc,
                        _normalize_for_serialization(vectorizer_config),
                        _normalize_for_serialization(model_config),
                    )
    ...
    def _save_predictions(
        ...
        rows = []
        test_row_ids = _slice_list(view.dataset.row_ids, view.split_indices.test_indices)
        true_labels = _slice_list(view.dataset.labels, view.split_indices.test_indices)
        for row_id, true_label, predicted_label in zip(test_row_ids, true_labels, predictions.tolist()):
            rows.append(
                {
                    "row_id": row_id,
    ...
    def _materialize_best_result(
        ...
        feature_set = self._build_feature_set(view=view, vectorizer_config=vectorizer_config)
        estimator = self._build_estimator(model_name=trial.model_name, model_config=model_config)
