...
PIPELINE_VERSION = "sentiment140_supervised_tweet_aware_v4_balanced_v2"
...
def resolve_dataset_file(task_dir: Path) -> Path:
    search_roots = (task_dir.parent, task_dir)
    candidate_name_set = {name.lower() for name in DATASET_CANDIDATE_NAMES}

    for search_root in search_roots:
        for candidate_name in DATASET_CANDIDATE_NAMES:
    ...
    for search_root in search_roots:
        for candidate in search_root.rglob("*"):
            if candidate.is_file() and candidate.name.lower() in candidate_name_set:
                return candidate

    raise FileNotFoundError(f"Could not find {DEFAULT_DATASET_FILENAME}.")
...
def resolve_dataset_columns(dataset_file: Path) -> tuple[str, str]:
    preview_df = pd.read_csv(dataset_file, nrows=0, encoding="utf-8")
    columns = {str(column).strip().lower(): str(column) for column in preview_df.columns}
    text_col = next((columns[key] for key in ("text", "tweet", "content", "review")), None)
    label_col = next((columns[key] for key in ("label", "polarity", "sentiment", "target", "class")), None)
    if text_col is None or label_col is None:
        available = ", ".join(preview_df.columns.astype(str).tolist())
        raise RuntimeError(
            f"{dataset_file.name} must include text and label columns. Found columns: {available or '<none>'}."
        )
    return text_col, label_col


def load_sentiment_dataset(dataset_file: Path, sample_size: int | None, random_seed: int) -> SentimentDataset:
    text_col, label_col = resolve_dataset_columns(dataset_file)
    df = pd.read_csv(dataset_file, usecols=[text_col, label_col], encoding="utf-8")
    df[label_col] = pd.to_numeric(df[label_col], errors="coerce")
    df = df.dropna(subset=[label_col]).copy()
    df[label_col] = df[label_col].astype(int)
    df[text_col] = df[text_col].fillna("").astype(str).map(normalize_text)
    df = df[df[text_col].str.len() > 0].copy()

    available_rows = int(len(df))
    if available_rows < 100:
        raise RuntimeError(f"Too few labeled rows available in {dataset_file.name}.")
    ...
    label_values = sorted(int(value) for value in df[label_col].unique().tolist())
    if len(label_values) < 2:
        raise RuntimeError(f"{dataset_file.name} must contain at least two sentiment classes.")

    label_to_index = {label: idx for idx, label in enumerate(label_values)}
    labels = df[label_col].map(label_to_index).to_numpy(dtype=np.int64)
    class_distribution = {label: int((df[label_col] == label).sum()) for label in label_values}
...
def write_report(
    ...
    lines = [
        "# Task2 - Sentiment Classification with Dense and Recurrent Models",
        "",
        "## Setup",
        "",
        f"- Dataset: `{dataset_file}`",
        f"- Source dataset file: `{dataset_file.name}`",
        f"- Samples used in this run: **{len(dataset.docs)}**",
        f"- Sampling mode: **{sampling_mode}**",
        f"- Gold labels are read directly from the `{dataset.label_column}` column and remapped internally to contiguous class IDs for PyTorch training.",
        ...
        "- Pretrained sequence embeddings are fine-tuned during sentiment training (`freeze=False`) so the model can adapt them to the cleaned Sentiment140 subset.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task2: sentiment classification with dense and recurrent model comparisons")
    parser.add_argument(
        "--sample-size",
        type=int,
        default=DEFAULT_SAMPLE_SIZE,
        help=(
            "Maximum number of rows to use from the resolved sentiment dataset "
            f"(defaults to {DEFAULT_DATASET_FILENAME}); set to 0 to use the full dataset."
        ),
    )
...
def main() -> None:
    ...
    csv_file = output_dir / "task2_results.csv"
    ...
    if (
        ...
        print("Using cached Task2 models and previous metrics.")
